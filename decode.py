...
def maindecode(stegimage,output_path):
	
	decoded_image = decode(Image.open(stegimage))
...
def reconstruct_image(image_pixels, width, height):
	...
	for col in range(width):
		for row in range(height):
			...
			print(r_binary,g_binary,b_binary,"hai" , len(r_binary),len(g_binary),len(b_binary))
			print(type(r_binary),type(g_binary),type(b_binary))
			if len(r_binary)==0:
				r_binary="00000000"
			if len(g_binary)==0:
				g_binary="00000000"
